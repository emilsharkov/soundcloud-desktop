...
export class AudioTransport {
  ...
  setEndedHandler = (fn: (() => void) | null) => { this.endedHandler = fn; };
  ...
  attach = (el: HTMLAudioElement | null) => {
    ...
    if (this.el) {
      ...
      this.duration = Number.isFinite(this.el.duration) ? this.el.duration : 0;
  ...
  private onPlay = () => { this.paused = false; this.emit(); };
  private onPause = () => { this.paused = true; this.emit(); };
  private onEnded = () => { if (this.endedHandler) this.endedHandler(); };
  ...
  private detachFromCurrentElement() {
    if (this.el) {
      ...
      this.el.removeEventListener("durationchange", this.onDurationChange);
  ...
  setPaused = async (paused: boolean) => {
    if (!this.el) { this.paused = paused; this.emit(); return; }
    if (paused) this.el.pause();
    else {
      try { await this.el.play(); } catch { /* autoplay may fail */ }
    }
