...
export class PlayerQueue {
  ...
  getSnapshot = (): QueueSnapshot => ({
    ...
    selectedTrackId: this.currentIndex >= 0 ? this.tracks[this.currentIndex]?.id?.toString() ?? null : null,
  });

  /** Helpers */
  private makeSequential(n: number) { return Array.from({ length: n }, (_, i) => i); }
  ...
  toggleShuffle() { this.setShuffled(!this.shuffled); }
  setRepeat(r: Repeat) { this.repeat = r; this.emit(); }

  next() {
    const n = this.tracks.length;
    if (!n) return;
    if (this.repeat === "song") { this.emit(); return; }
